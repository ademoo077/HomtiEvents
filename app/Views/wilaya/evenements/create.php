 ...
 *  @var array $categories @var array $epics @var array $selectedAnomalies @var array $assignedEpics
...
#eventMap { height: 320px; border-radius: 0 0 .375rem .375rem; z-index: 1; }
...
<div class="wh-page">
        ...
        <div class="wh-hero-inner">
            <div class="wh-hero-row">
                <div class="wh-hero-text">
                    <h1 class="wh-hero-title"><i class="mdi mdi-calendar-plus me-2"></i><?= e(__('evenements.create')) ?></h1>
                    <p class="wh-hero-sub"><?= e(__('common.informations')) ?></p>
                </div>
                <div class="wh-hero-actions">
                    <a href="<?= url('wilaya/evenements') ?>" class="btn btn-sm btn-outline-light">
    ...
    <form method="post" action="<?= url('wilaya/evenements') ?>" novalidate>
        ...
        <div class="card border-0 shadow-sm mb-4 event-map-card">
            <div class="card-header d-flex justify-content-between align-items-center">
                ...
                <div class="d-flex align-items-center gap-2">
                    <input type="hidden" id="latitude" name="latitude" value="<?= e((string) $oldVal('latitude', $event['latitude'] ?? '')) ?>">
                    <input type="hidden" id="longitude" name="longitude" value="<?= e((string) $oldVal('longitude', $event['longitude'] ?? '')) ?>">
            ...
            <div class="card-body p-3">
                <div class="row g-3">
                    <div class="col-md-6">
                        ...
                        <select class="form-select <?= isset($errors['commune_id']) ? 'is-invalid' : '' ?>" id="commune_id" name="commune_id" required>
                            ...
                            <?php foreach ($communes as $c): ?>
                                <option value="<?= e((string) $c['id']) ?>" data-lat="<?= e((string) ($c['latitude'] ?? '')) ?>" data-lng="<?= e((string) ($c['longitude'] ?? '')) ?>" <?= (string) $oldVal('commune_id', $event['commune_id'] ?? '') === (string) $c['id'] ? 'selected' : '' ?>><?= e($c['nom']) ?></option>
                            <?php endforeach; ?>
                        </select>
                        <?= $error('commune_id') ?>
                    </div>
                    ...
                    <div class="col-12">
                        <small class="text-muted">Cliquez sur la carte pour placer un marqueur, ou utilisez le bouton cibler pour votre position actuelle.</small>
                    </div>
                </div>
            </div>
        </div>
                ...
                <div class="row g-3">
                    ...
                    <div class="col-md-6">
                        ...
                        <input type="number" class="form-control <?= isset($errors['capacite']) ? 'is-invalid' : '' ?>" id="capacite" name="capacite"
                               min="1" step="1" value="<?= e((string) $oldVal('capacite', $event['capacite'] ?? '')) ?>" placeholder="200">
                        <?= $error('capacite') ?>
                    </div>
                    <div class="col-12">
        ...
        <!-- Anomalies multi-sélection avec GPS par anomalie -->
        <div class="card border-0 shadow-sm mb-4">
            <div class="card-header d-flex justify-content-between align-items-center">
                <span><i class="mdi mdi-alert-octagon me-2"></i><?= e(__('evenements.anomalies')) ?> *</span>
                <small class="text-muted">Sélectionnez les anomalies et géolocalisez-les individuellement</small>
            </div>
            <div class="card-body">
                <div id="anomalyList">
                    <?php foreach ($anomalies as $an): ?>
                        <?php
                        $isChecked = in_array((int) $an['id'], $selectedAnomalies, true);
                        ?>
                        <div class="anomaly-card <?= $isChecked ? 'anomaly-selected' : '' ?>" data-anomaly-id="<?= (int) $an['id'] ?>">
                            <div class="anomaly-header">
                                <input class="form-check-input anomaly-toggle" type="checkbox" name="anomalies[]" id="anomalie-<?= (int) $an['id'] ?>"
                                       value="<?= (int) $an['id'] ?>" <?= $isChecked ? 'checked' : '' ?>>
                                <label class="form-check-label fw-semibold" for="anomalie-<?= (int) $an['id'] ?>"><?= e($an['nom']) ?></label>
                                <?php if (!empty($an['icone'])): ?>
                                    <i class="<?= e($an['icone']) ?>" style="color: <?= e($an['couleur'] ?? '#6366f1') ?>; margin-left: auto;"></i>
                                <?php endif; ?>
                            </div>
                            <div class="anomaly-gps">
                                <div class="gps-inputs">
                                    <div class="form-floating">
                                        <input type="number" step="any" class="form-control form-control-sm" name="anomaly_lat[<?= (int) $an['id'] ?>]"
                                               placeholder="Latitude" value="<?= e((string) $oldVal('anomaly_lat_' . $an['id'], '')) ?>">
                                        <label>Latitude</label>
                                    </div>
                                    <div class="form-floating">
                                        <input type="number" step="any" class="form-control form-control-sm" name="anomaly_lng[<?= (int) $an['id'] ?>]"
                                               placeholder="Longitude" value="<?= e((string) $oldVal('anomaly_lng_' . $an['id'], '')) ?>">
                                        <label>Longitude</label>
                                    </div>
                                    <button type="button" class="btn btn-sm btn-outline-primary btn-set-pin" data-anomaly-id="<?= (int) $an['id'] ?>" title="Pointer sur la carte">
...
(function(){
    ...
    // Set pin for anomaly from main marker
    ...
    function fetchRoutingPreview() {
        ...
        routingTimer = setTimeout(function() {
            ...
            const anomalyIds = [];
            document.querySelectorAll('.anomaly-toggle:checked').forEach(function(cb) {
                anomalyIds.push(cb.value);
            });
            if (!communeId || anomalyIds.length === 0) {
            ...
            fetch('<?= url("wilaya/api/routing-preview") ?>?commune_id=' + communeId + '&anomalies[]=' + anomalyIds.join('&anomalies[]='),
                ...
                if (!data.ok || !data.preview || data.preview.length === 0) {
                    document.getElementById('routingPreview').style.display = 'none';
                    return;
                }
                let html = '<div class="mt-2"><strong><i class="mdi mdi-route me-1"></i>Aperçu du routage automatique :</strong></div>';
                const epicMap = {};
                ...
                if (Object.keys(epicMap).length > 0) {
                    html += '<div class="routing-preview-box mt-2"><strong>EPIC(s) suggérée(s) :</strong> ';
                    Object.values(epicMap).forEach(function(n) { html += '<span class="badge bg-primary me-1">' + n + '</span>'; });
                    html += '</div>';
                } else {
                    html += '<div class="routing-preview-box no-match mt-2"><i class="mdi mdi-alert-circle me-1"></i>Aucune règle de routage ne correspond. Une alerte sera créée.</div>';
                }
